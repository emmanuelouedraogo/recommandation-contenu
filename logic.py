...
def charger_df_depuis_blob(blob_name: str) -> pd.DataFrame:
    ...
    def _download_with_retry(client):
        ...
        max_retries = 3
        backoff_seconds = 1  # Initial backoff
    ...
    try:
        ...
        if not interactions_df.empty:
            ...
            if not user_interactions.empty:
                ...
                user_interactions = user_interactions.rename(
                    columns={"rating": "score"}
                )  # Renommer pour correspondre au format de l'API
...
def obtenir_tous_les_utilisateurs_avec_statut():
    ...
    try:
        ...
        logger.warning(
            f"Le fichier principal des clics '{CLICKS_BLOB_NAME}' n'a pas été trouvé. On continue avec une liste vide."
        )
    ...
    try:
        ...
        logger.warning(
            f"Le fichier des utilisateurs '{USERS_BLOB_NAME}' n'a pas été trouvé. On continue avec une liste vide."
        )
...
def creer_nouvel_article(title: str, content: str, category_id: int) -> int:
    ...
    append_blob_client = blob_service_client.get_blob_client(
        ...
        blob=NEW_ARTICLES_LOG_BLOB_NAME,
    )
