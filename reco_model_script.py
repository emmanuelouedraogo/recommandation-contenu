# -*- coding: utf-8 -*-
"""
Script d'Entraînement et de Déploiement du Modèle de Recommandation

Ce script exécute le pipeline complet de Machine Learning :
1. Chargement des données depuis Azure Blob Storage.
2. Préparation des données et division en ensembles d'entraînement/test.
3. Entraînement d'un modèle hybride (Collaboratif + Contenu).
4. Sauvegarde du modèle entraîné dans un fichier .pkl pour le déploiement.
"""
...
# import mlflow # Désactivé pour le script de production simple
...
def cut_data_by_qtime(clics, items, items2vec, qtime=0.5):
    """
...
# --- Définition des Classes de Modèles ---
...
def main():
    """
    Exécute le pipeline complet d'entraînement du modèle.
    """
    ...
    try:
        ...
        print(f"Chargement terminé. {len(clics)} clics, {len(items)} articles.")
    ...
    # Utiliser toutes les données disponibles pour l'entraînement final
    data_map, items_df, i2vec, dic_ir, dic_ri = cut_data_by_qtime(clics, items, items2vec, qtime=1.0)

    # Les hyperparamètres optimaux (déterminés lors de l'exploration)
    # Ces valeurs peuvent être stockées dans un fichier de configuration
    best_cf_weight = 0.2
    best_cb_weight = 0.8
    best_cb_decay_rate = 0.05

    print(f"Utilisation des hyperparamètres : cf_weight={best_cf_weight}, cb_weight={best_cb_weight}, decay_rate={best_cb_decay_rate}")

    # --- 3. Entraînement du modèle final ---
    print("\n--- Entraînement du modèle final avec les meilleurs hyperparamètres ---")
    final_hybrid_model = HybridRecommender(data_map, i2vec, dic_ri, dic_ir, items_df,
                                           cf_weight=best_cf_weight, cb_weight=best_cb_weight)

    final_cf_model = CollabFiltRecommender(data_map) # SVDpp
    final_cb_model = ContentBasedTimeDecayRecommender(data_map, i2vec, dic_ri, dic_ir, decay_rate=best_cb_decay_rate)
    final_pf_model = PopularityFiltRecommender(data_map)
    ...
    print(f"Modèle final sauvegardé localement dans '{model_path}'.")
