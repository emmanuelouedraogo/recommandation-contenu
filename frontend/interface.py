...
def charger_df_depuis_blob(blob_name: str) -> pd.DataFrame:
    ...
    Cette fonction est mise en cache. Elle récupère le client de service blob qui est lui-même mis en cache.
    """
    ...
    try:
        ...
        st.error(
            f"Erreur de connexion au stockage Azure. Vérifiez votre connexion internet et la chaîne de connexion. Erreur: {e}"
        )
        return pd.DataFrame()
    ...
    with st.spinner("Recherche de vos recommandations..."):
        try:
            ...
            response = requests.get(
                f"{api_url}/api/recommend", params={"user_id": user_id}, headers=headers, timeout=20
            )
            ...
            st.error(error_msg)
            logger.error(
                f"Erreur inattendue lors de la récupération des recommandations pour user_id {user_id}. {error_msg}"
            )
            return None


def afficher_page_recommandations(api_url):
    ...
    if st.session_state.user_id is None:
        ...
        recommendations = obtenir_recommandations(api_url, user_id)
        ...
        if clicks_df.empty:
            ...
            if user_history_df.empty:
                ...
                if not all(col in user_history_df.columns for col in required_cols):
                    st.error(
                        "Le fichier d'historique (clicks_sample.csv) est mal formaté. Il manque des colonnes attendues (ex: 'user_id', 'article_id')."
                    )
                    logger.error(
                        f"Colonnes manquantes dans clicks_sample.csv. Colonnes trouvées : {user_history_df.columns.tolist()}"
                    )
                    st.stop()

                user_history_df = user_history_df.sort_values("click_timestamp").drop_duplicates(
                    subset=["user_id", "article_id"], keep="last"
                )
                articles_df_history = charger_df_depuis_blob(ARTICLES_BLOB_NAME)
                history_details = user_history_df.merge(articles_df_history, on="article_id", how="left").fillna(
                    {"title": "Titre inconnu"}
                )
                ...
                for _, row in history_details.iterrows():
                    ...
                    with col1:
                        ...
                        st.caption(
                            f"Dernière modification : {pd.to_datetime(row['click_timestamp'], unit='s').strftime('%Y-%m-%d %H:%M')}"
                        )
                    with col2:
                        new_rating = st.number_input(
                            ...
                            min_value=1,
                            max_value=5,
                            value=int(row.get("nb", 0)),
                            key=f"update_rating_{row['article_id']}",
                        )
                        if st.button(
                            "Modifier la note", key=f"update_btn_{row['article_id']}", use_container_width=True
                        ):
...
def main():
    ...
    try:
        ...
        # Le client blob est récupéré à la volée par les fonctions qui en ont besoin.
    ...
    if choice == "Recommandations":
        afficher_page_recommandations(api_url)
